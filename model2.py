...
class Transformer(object):
    ...
    def __init__(self, hp, wordEmbedding):

        # 定义模型的输入
        self.hp = hp
        self.inputX = tf.placeholder(tf.int32, [None, hp.sequence_length], name="inputX")
        self.inputY = tf.placeholder(tf.int32, [None], name="inputY")

        self.dropoutKeepProb = tf.placeholder(tf.float32, name="dropoutKeepProb")
        self.embeddedPosition = tf.placeholder(tf.float32, [None, hp.sequence_length, hp.sequence_length],
            ...
            for i in range(hp.num_blocks):
                with tf.name_scope("transformer-{}".format(i + 1)):
                    ...
                    self.embeddedWords = self._feedForward(multiHeadAtt,
                                                           [hp.d_ff,
                                                            hp.d_model + hp.sequence_length])

            outputs = tf.reshape(self.embeddedWords,
                                 [-1, hp.sequence_length * (hp.d_model + hp.sequence_length)])
        ...
        with tf.name_scope("output"):
            outputW = tf.get_variable(
                "outputW",
                shape=[outputSize, hp.num_class],
                initializer=tf.contrib.layers.xavier_initializer())

            outputB = tf.Variable(tf.constant(0.1, shape=[hp.num_class]), name="outputB")
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = tf.reduce_mean(losses) + hp.l2RegLambda * l2Loss
